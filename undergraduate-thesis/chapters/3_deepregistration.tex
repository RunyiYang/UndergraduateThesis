...
\section{标准数据集}
本节将介绍用于点云配准的标准数据集。在评估不同指标的性能时，本文需要用到不同的公开数据集。点云配准任务的数据集大致可以分为仿真数据集和真实数据集。仿真数据集中的物体对象是完整的三维模型，比如ModelNet40\cite{sunmodelnet40} 。而真实数据集中，因为存在遮挡、测量误差、相干光干扰、背景影响等问题，往往会存在缺失、噪声、过密/稀疏等问题，比如Scan2CAD\cite{avetisyan2019scan2cad} 和ShapeNet\cite{chang2015shapenet} 。下表 \ref{tab:PointCloudDatasets} 列出了常用的点云配准数据集。
\begin{table}[h!]
    \centering
    \vspace{-1.0cm}
    ...
    McGill Benchmark \cite{siddiqi2008retrieving} & 2008 & 456 & 19 & 304 & 152 & 仿真场景 & 三角网格 \\ 
    Sydney Urban Objects \cite{de2013unsupervised} & 2013 & 588 & 14 & - & - & 真实场景 & 点云 \\ 
    ModelNet10 \cite{wu20153d} & 2015 & 4899 & 10 & 3991 & 605 & 仿真场景 & 三角网格 \\ 
    ModelNet40 \cite{sunmodelnet40} & 2015 & 12311 & 40 & 9843 & 2468 & 仿真场景 & 三角网格 \\ 
    ShapeNet \cite{chang2015shapenet} & 2015 & 51190 & 55 & - & - & 仿真场景 & 三角网格 \\ 
    ScanNet \cite{dai2017scannet} & 2017 & 12283 & 17 & 9677 & 2606 & 真实场景 & RGB-D \\ 
    ScanObjectNN \cite{uy2019revisiting} & 2016 & 2902 & 15 & 2321 & 581 & 真实场景 & 三角网格 \\ 
    S3DIS \cite{armeni20163d} & 2017 & 271 & 13 & - & - & 真实场景 & 点云 \\ 
    7Scenes \cite{shotton2013scene} & 2013 & 7 & 1 & 26000 & 17000 & 真实场景 & RGB-D \\ 
...
在本次任务中，本文使用ModelNet40、ShapeNet、Scan2CAD组成多实例点云配准仿真数据集进行训练和测试。所以，本文对这3个数据集和多实例点云配准数据集进行了详细的介绍。
...
图卷积神经网络：在连接两个特征编码之前，本文首先使用图神经网络 (GNN) 进一步聚合并强化它们各自的上下文关系。首先，使用 k-NN 方法将 $\boldsymbol{P'}$ 中的超点链接成一个欧几里得空间图。设 $\boldsymbol{x_i \in \mathbb{R}^b}$ 表示超点 $\boldsymbol{p'_i}$ 的特征编码，$\boldsymbol{(i, j) \in E}$ 表示超点 $\boldsymbol{p'_i}$ 和 $\boldsymbol{p'_j}$ 之间的图边。然后，编码器特征按照以下方式迭代更新：
\begin{equation}
